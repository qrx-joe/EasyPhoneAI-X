...
describe('ERROR_CODES — 方案 §8.2 九个稳定错误码', () => {
  test('INVALID_INPUT', () => {
  ...
  test('VISION_TIMEOUT', () => {
    assert.equal(ERROR_CODES.VISION_TIMEOUT, 'VISION_TIMEOUT')
  })
  ...
  test('恰好九个错误码（方案 §8.2）', () => {
    ...
    assert.equal(values.length, 9, `应有 9 个错误码，实际 ${values.length}`)
  })
